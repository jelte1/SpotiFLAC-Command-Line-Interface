...
def safe_int(value) -> int:
    """Converte para int de forma segura, retornando 0 se falhar."""
    try:
        return int(value)
...
class AmazonDownloader:
    ...
    def download_from_afkar_xyz(self, amazon_url: str, output_dir: str) -> str:
        ...
        # Caso não haja chave, renomeia o arquivo baixado
    ...
    def download_by_url(self, amazon_url: str, output_dir: str, quality: str, filename_format: str, 
        ...
        # 2. Renomeia com base nos metadados do Spotify
        ...
        # 3. Incorpora Metadados (Capa + Tags)
        # Nota: Usamos os dados vindos do Spotify (argumentos da função)
    ...
    def embed_metadata(self, filepath, title, artist, album, album_artist, date, track_num, total_tracks, 
        ...
        try:
            ...
            # Ajuste para garantir que 0 vire 1 se necessário, ou mantenha coerência
            if t_num == 0: t_num = 1
            if d_num == 0: d_num = 1
            ...
            if filepath.endswith(".flac"):
                ...
                try:
                    ...
                    audio = MP4(filepath) # Retry ou criação nova se corrompido
    ...
    def download_by_spotify_id(self, spotify_track_id, **kwargs):
        ...
        # Define valores padrão para metadados caso o main.py não envie algum
        default_kwargs = {
        ...
        # Atualiza com o que veio do main.py
        for key in kwargs:
